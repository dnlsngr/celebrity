...
describe('NewRound', () => {
  it('should render NewRound with proper roundNumber', () => {
    ...
    expect(wrapper.find('[data-test="round-start-header"]').text()).to.equal("Let's start round 1")
  })
  ...
  it('should show names missed if they are provided', () => {
    ...
    expect(wrapper.find('[data-test="names-missed"]').childAt(1).html()).to.contain(NAME_MISSED)
  })
  ...
  it('should start turn when clicking Begin Round', () => {
    ...
    const wrapper = mount(<NewRound roundInfo={roundInfoProps} readyForTurn={readyForTurnStub} />)
    const beginRoundButton = wrapper.find('[data-test="begin-round-button"]').find('.ant-btn')
