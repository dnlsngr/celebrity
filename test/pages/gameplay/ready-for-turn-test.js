...
describe('ReadyForTurn', () => {
  it('should render ReadyForTurn with scores', () => {
    ...
    const wrapper = shallow(<ReadyForTurn scores={scores} roundInfo={roundInfo} />)
  ...
  it('should call beginTurn when button is clicked', () => {
    ...
    const wrapper = mount(<ReadyForTurn scores={[]} roundInfo={roundInfo} beginTurn={beginTurnStub} />)
    wrapper.find('[data-test="begin-turn-button"]').find('.ant-btn').simulate('click')
