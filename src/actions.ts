...
const finalizePlayers = (state: CelebrityReduxState) => {
  ...
  const numPlayers =
    state.gameSetup.currentNames.length > 0
...
const processName = (result: string, state: CelebrityReduxState) => {
  ...
  const skippedOrIllegalNames =
    result === SKIP || result === ILLEGAL
  ...
  return {
    ...
    turnInfo: {
      ...
      correctThisTurn:
        result === CORRECT ? correctThisTurn + 1 : correctThisTurn,
      skippedThisTurn: result === SKIP ? skippedThisTurn + 1 : skippedThisTurn,
      illegalThisTurn:
        result === ILLEGAL ? illegalThisTurn + 1 : illegalThisTurn
    }
  };
};
...
const endTurn = (state: CelebrityReduxState) => {
  ...
  const updatedScoreState = _reconcileResults(
    state,
    isEndOfTurnset,
...
const _reconcileResults = (
  state: CelebrityReduxState,
  isEndOfTurnset: boolean,
  ...
  const team1 = isTeam1Turn
    ? state.scores.team1 + state.turnInfo.correctThisTurn
    : state.scores.team1;
  const team2 = !isTeam1Turn
    ? state.scores.team2 + state.turnInfo.correctThisTurn
    : state.scores.team2;
  return {
    ...state,
    scores: {
